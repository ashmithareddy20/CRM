...
const pathSchema = z.object({ leadId: text, evidenceId: text, path: z.enum(["surgical", "medical_management"]) }).strict();
...
export async function handleClinicalRoutes(request: Request, context: RequestContext, dependencies: ClinicalRouteDependencies): Promise<Response | undefined> {
  ...
  if (path === "/api/v1/procedure-bookings") { const body = parse(z.object({ leadId: text, evidenceId: text }).strict(), await readJsonBody(request)); return response(context, await dependencies.clinical.recordProcedureBooking(clinicalContext(context), body.leadId, body.evidenceId), 201); }
  if (path === "/api/v1/admissions") { const body = parse(z.object({ leadId: text, evidenceId: text }).strict(), await readJsonBody(request)); return response(context, await dependencies.clinical.recordAdmission(clinicalContext(context), body.leadId, body.evidenceId), 201); }
  if (path === "/api/v1/treatments") return response(context, await dependencies.clinical.completeTreatment(clinicalContext(context), parse(pathSchema, await readJsonBody(request))), 201);
  return undefined;
}
