...
export interface DiagnosisRecord extends Omit<DiagnosisInput, "detailedRemark" | "evidenceId"> { id: string; tenantId: string; evidenceId: string; status: "active" | "superseded"; createdAt: Date; createdByMembershipId: string; }
...
export interface DiagnosisFinding { tenantId: string; diagnosisId: string; finding: string; evidenceId: string; ownerMembershipId: string; reviewAt: Date; createdAt: Date; createdByMembershipId: string; }
export interface DiagnosisRepository {
  getLead(tenantId: string, leadId: string): Promise<{ id: string; contactId: string; lifecycleStage: string } | undefined>;
  /** Atomically supersedes a prior active diagnosis, inserts this diagnosis, and records its management finding. */
  supersedeAndAdd(record: DiagnosisRecord, finding: DiagnosisFinding): Promise<void>;
...
export class DiagnosisService {
  ...
  async record(context: DiagnosisContext, input: DiagnosisInput): Promise<{ state: "recorded"; diagnosis: DiagnosisRecord; actions: readonly CorrectiveAction[] } | { state: "evidence_pending"; task: DiagnosisTask }> {
    ...
    const diagnosis: DiagnosisRecord = { id: id(), tenantId: context.tenantId, leadId: input.leadId, versionId: input.versionId, primaryReason: input.primaryReason, secondaryReason: input.secondaryReason, evidenceId: input.evidenceId, recoverability: input.recoverability, responsibleMembershipId: input.responsibleMembershipId, reviewAt: input.reviewAt, status: "active", createdAt: context.now, createdByMembershipId: context.actorMembershipId };
    const actions = CORRECTIVE_ACTIONS[input.primaryReason];
    const finding: DiagnosisFinding = { tenantId: context.tenantId, diagnosisId: diagnosis.id, finding: `Diagnosis: ${input.detailedRemark.trim()} | Recommended actions: ${actions.map((action) => action.code).join(", ")}`, evidenceId: input.evidenceId, ownerMembershipId: input.responsibleMembershipId, reviewAt: input.reviewAt, createdAt: context.now, createdByMembershipId: context.actorMembershipId };
    await this.repository.supersedeAndAdd(diagnosis, finding);
    return { state: "recorded", diagnosis, actions };
  }
}
...
export class MemoryDiagnosisRepository implements DiagnosisRepository {
  ...
  readonly diagnoses: DiagnosisRecord[] = []; readonly tasks: DiagnosisTask[] = []; readonly findings: DiagnosisFinding[] = [];
  async getLead(_tenantId: string, leadId: string) { return this.leads.get(leadId); }
  async supersedeAndAdd(record: DiagnosisRecord, finding: DiagnosisFinding) { for (const prior of this.diagnoses) if (prior.tenantId === record.tenantId && prior.leadId === record.leadId && prior.status === "active") prior.status = "superseded"; this.diagnoses.push(record); this.findings.push(finding); }
...
export class D1DiagnosisRepository implements DiagnosisRepository {
  ...
  async supersedeAndAdd(record: DiagnosisRecord, finding: DiagnosisFinding) {
    const at = record.createdAt.getTime();
    await this.db.batch([
      this.db.prepare("UPDATE crm_lead_diagnoses SET status = 'superseded', updated_at = ?, updated_by_membership_id = ?, version = version + 1 WHERE tenant_id = ? AND lead_id = ? AND status = 'active'").bind(at, record.createdByMembershipId, record.tenantId, record.leadId),
      this.db.prepare("INSERT INTO crm_lead_diagnoses (id, tenant_id, lead_id, version_id, primary_reason, secondary_reason, evidence_id, recoverability, review_at, status, created_at, created_by_membership_id, version) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, 'active', ?, ?, 1)").bind(record.id, record.tenantId, record.leadId, record.versionId, record.primaryReason, record.secondaryReason, record.evidenceId, record.recoverability, record.reviewAt.getTime(), at, record.createdByMembershipId),
      this.db.prepare("INSERT INTO crm_management_findings (id, tenant_id, diagnosis_id, finding, evidence_id, owner_membership_id, review_at, created_at, created_by_membership_id, version) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, 1)").bind(id(), finding.tenantId, finding.diagnosisId, finding.finding, finding.evidenceId, finding.ownerMembershipId, finding.reviewAt.getTime(), finding.createdAt.getTime(), finding.createdByMembershipId),
    ]);
  }
