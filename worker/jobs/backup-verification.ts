...
export async function verifyIsolatedRestore(input: { manifest: BackupManifest; env: { FIELD_ENCRYPTION_KEYS?: string; FIELD_ENCRYPTION_ACTIVE_VERSION?: string }; restoreDatabase: (key: string, sha256: string) => Promise<void>; restoreObject: (key: string, sha256: string) => Promise<void>; suppressions: readonly SuppressionReplayLedger[]; replaySuppression: (entry: SuppressionReplayLedger) => Promise<void>; now?: Date }): Promise<BackupVerification> {
