...
const files = canonical || knownLegacy ? ["drizzle/forward/0000_moaning_cammi.sql", "drizzle/forward/0001_tenant_guards.sql", "drizzle/forward/0002_domain_guards.sql"] : ["drizzle/baseline/0000_canonical_baseline.sql", "drizzle/baseline/0001_tenant_guards.sql", "drizzle/baseline/0002_domain_guards.sql"];
for (const file of files) {
  const sql = await readFile(file, "utf8");
  for (const statement of sql.split(/--\s*>\s*statement-breakpoint\s*\n/).map((part) => part.trim()).filter(Boolean)) {
    try { db.exec(statement); } catch (error) { if (!String(error.message).includes("already exists")) throw error; }
  }
}
