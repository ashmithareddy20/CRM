...
import type { ChannelProviderAdapter } from "../../providers/contracts";
...
export interface MessageAttempt { id: string; tenantId: string; touchId?: string; contactId: string; channel: MessageChannel; purpose: string; templateVersionId?: string; acceptedContentHash?: string; provider?: string; providerMessageId?: string; status: MessageStatus; acceptedAt?: Date; createdAt: Date; }
...
export interface DispatchRepository {
  ...
  hasContentHash(tenantId: string, contactId: string, hash: string): Promise<boolean>;
...
export interface DispatchInput { tenantId: string; contactId: string; touchId: string; purpose: string; requestedChannel: MessageChannel | "rich"; templateVersion: TemplateVersion; integrationId: string; provider: ChannelProviderAdapter; now: Date; operationId: string; }
...
export class CommunicationService {
  constructor(private readonly repository: DispatchRepository, private readonly consents: ConsentService, private readonly minimumIntervalMs = MIN_OUTBOUND_INTERVAL_MS) {}
  ...
  async dispatch(input: DispatchInput): Promise<DispatchResult> {
    ...
    if (await this.repository.hasContentHash(input.tenantId, input.contactId, input.templateVersion.contentHash)) return this.blocked(input, "content_repetition", channel);
    ...
    try {
      ...
      attempt.status = "delivery_unknown"; await this.repository.updateAttempt(attempt); await this.repository.releaseGate(input.tenantId, input.contactId, input.touchId, input.now);
  ...
  async recordEvent(input: Omit<MessageEvent, "id">): Promise<boolean> {
    return this.repository.appendEvent({ ...input, id: id() });
  }
...
export class MemoryDispatchRepository implements DispatchRepository {
  ...
  async hasContentHash(tenantId: string, contactId: string, hash: string) { return this.attempts.some((item) => item.tenantId === tenantId && item.contactId === contactId && item.acceptedContentHash === hash && item.status === "accepted"); }
