...
Reporting is evidence-based and tenant-scoped. Facts contain event identifiers, timestamps, operational dimensions, and evidence references—not patient names, contact details, remarks, or unbounded provider payloads. Lead denominators and numerators use only the canonical `leadId`; an event ID is never substituted as a lead identity. A `(tenant_id, source_event_id)` uniqueness constraint makes projection safe under at-least-once queue delivery. The projector retains source events and supports deterministic daily aggregate rebuilds, so late conversions and revenue reversals correct prior days rather than inflating totals. Aggregate watermarks are a stable `{ occurredAt, factId, sourceEventId, factCount }` replay cursor, not a lexicographic event-id guess.
...
All queries apply tenant scope before filters. Date, branch, owner, source, campaign, channel, disease, and treatment predicates are pushed into SQL before each deterministic keyset page; aggregate queries page all facts and do not silently truncate. A branch-scoped request excludes branchless facts explicitly, and appointments, messages, tasks, and manager queues apply the same branch predicate. Branch-limited actors cannot request another branch; non-manager users can only see their assigned/originally assigned evidence. Export requests are asynchronous `202` report runs, are audit logged, tenant/branch scoped, private, encrypted-store compatible, size-bounded to 10 MiB, and expire after 24 hours. The job rereads the durable report-run contract, regenerates the authorized report under its saved scope, writes the private CSV object, and records object key/size/expiry before a download can be offered. Direct identifiers are suppressed by default. CSV values beginning with `=`, `+`, `-`, `@`, tab, or carriage return are prefixed with an apostrophe and CSV-quoted to prevent formula injection. Download authorization must be checked again by the private-object delivery endpoint.
