...
/** Derived from contact, consent/suppression, call, outcome and clinician records at enrollment/dispatch time. */
...
export class D1RecoveryRepository implements RecoveryRepository {
  ...
  async getLeadState(tenantId: string, leadId: string) {
    const row = await this.db.prepare("SELECT l.id AS leadId, l.contact_id AS contactId, l.lifecycle_stage AS lifecycleStage, c.status AS contactStatus, EXISTS(SELECT 1 FROM crm_consent_events ce WHERE ce.tenant_id = l.tenant_id AND ce.contact_id = l.contact_id AND ce.channel IS NULL AND ce.occurred_at = (SELECT MAX(latest.occurred_at) FROM crm_consent_events latest WHERE latest.tenant_id = ce.tenant_id AND latest.contact_id = ce.contact_id AND latest.purpose = ce.purpose AND latest.channel IS NULL) AND ce.state IN ('withdrawn', 'denied')) OR EXISTS(SELECT 1 FROM crm_consent_events ce WHERE ce.tenant_id = l.tenant_id AND ce.contact_id = l.contact_id AND ce.channel IS NOT NULL AND ce.occurred_at = (SELECT MAX(latest.occurred_at) FROM crm_consent_events latest WHERE latest.tenant_id = ce.tenant_id AND latest.contact_id = ce.contact_id AND latest.purpose = ce.purpose AND latest.channel = ce.channel) AND ce.state IN ('withdrawn', 'denied')) AS optedOut, EXISTS(SELECT 1 FROM crm_suppressions s WHERE s.tenant_id = l.tenant_id AND s.contact_id = l.contact_id AND s.channel IS NULL AND s.occurred_at = (SELECT MAX(latest.occurred_at) FROM crm_suppressions latest WHERE latest.tenant_id = s.tenant_id AND latest.contact_id = s.contact_id AND latest.channel IS NULL) AND s.active = 1) OR EXISTS(SELECT 1 FROM crm_suppressions s WHERE s.tenant_id = l.tenant_id AND s.contact_id = l.contact_id AND s.channel IS NOT NULL AND s.occurred_at = (SELECT MAX(latest.occurred_at) FROM crm_suppressions latest WHERE latest.tenant_id = s.tenant_id AND latest.contact_id = s.contact_id AND latest.channel = s.channel) AND s.active = 1) AS doNotContact, (c.status != 'active' OR EXISTS(SELECT 1 FROM crm_call_attempts a WHERE a.tenant_id = l.tenant_id AND a.lead_id = l.id AND a.disposition IN ('invalid_number', 'wrong_number'))) AS invalid, EXISTS(SELECT 1 FROM crm_call_attempts a WHERE a.tenant_id = l.tenant_id AND a.lead_id = l.id AND a.disposition = 'rejected') AS rejected, EXISTS(SELECT 1 FROM crm_lead_outcomes o WHERE o.tenant_id = l.tenant_id AND o.lead_id = l.id AND o.outcome IN ('already_treated', 'treated_elsewhere')) AS alreadyTreated, EXISTS(SELECT 1 FROM crm_clinical_decisions d WHERE d.tenant_id = l.tenant_id AND d.lead_id = l.id AND d.decision = 'clinically_ineligible') AS clinicallyIneligible FROM crm_lead_episodes l JOIN crm_contacts c ON c.tenant_id = l.tenant_id AND c.id = l.contact_id WHERE l.tenant_id = ? AND l.id = ? AND l.archived_at IS NULL")
      .bind(tenantId, leadId).first<{ leadId: string; contactId: string; lifecycleStage: string; contactStatus: string; optedOut: number; doNotContact: number; invalid: number; rejected: number; alreadyTreated: number; clinicallyIneligible: number }>();
    if (!row) return undefined;
    return { ...row, optedOut: Boolean(row.optedOut), doNotContact: Boolean(row.doNotContact), invalid: Boolean(row.invalid), rejected: Boolean(row.rejected), alreadyTreated: Boolean(row.alreadyTreated), clinicallyIneligible: Boolean(row.clinicallyIneligible) };
  }
  async getActiveEnrollment(tenantId: string, leadId: string) {
    const row = await this.db.prepare("SELECT id, tenant_id AS tenantId, lead_id AS leadId, diagnosis_id AS diagnosisId, campaign_id AS campaignId, eligible_at AS eligibleAt, COALESCE(reactivation_at, eligible_at) AS reactivationAt, COALESCE(reason_snapshot, '') AS reasonSnapshot, status, created_at AS createdAt, created_by_membership_id AS createdByMembershipId FROM crm_recovery_enrollments WHERE tenant_id = ? AND lead_id = ? AND status IN ('planned', 'active', 'paused') LIMIT 1")
      .bind(tenantId, leadId).first<Omit<RecoveryEnrollment, "eligibleAt" | "reactivationAt" | "createdAt"> & { eligibleAt: number; reactivationAt: number; createdAt: number }>();
    return row ? { ...row, eligibleAt: new Date(row.eligibleAt), reactivationAt: new Date(row.reactivationAt), createdAt: new Date(row.createdAt) } : undefined;
  }
  ...
  async addEnrollment(e: RecoveryEnrollment) {
    try {
      const inserted = await this.db.prepare("INSERT INTO crm_recovery_enrollments (id, tenant_id, lead_id, diagnosis_id, campaign_id, eligible_at, reactivation_at, reason_snapshot, status, created_at, created_by_membership_id, version) SELECT ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, 1 WHERE NOT EXISTS (SELECT 1 FROM crm_recovery_enrollments WHERE tenant_id = ? AND lead_id = ? AND status IN ('planned', 'active', 'paused'))")
        .bind(e.id, e.tenantId, e.leadId, e.diagnosisId, e.campaignId, e.eligibleAt.getTime(), e.reactivationAt.getTime(), e.reasonSnapshot, e.status, e.createdAt.getTime(), e.createdByMembershipId, e.tenantId, e.leadId).run();
      return inserted.meta.changes === 1;
    } catch (error) {
      // The database active-enrollment constraint is the final concurrency authority.
      if (isConstraintConflict(error)) return false;
      throw error;
    }
  }
  ...
  async listDue(tenantId: string, now: Date, limit: number) {
    const rows = await this.db.prepare("SELECT id, tenant_id AS tenantId, lead_id AS leadId, diagnosis_id AS diagnosisId, campaign_id AS campaignId, eligible_at AS eligibleAt, COALESCE(reactivation_at, eligible_at) AS reactivationAt, COALESCE(reason_snapshot, '') AS reasonSnapshot, status, created_at AS createdAt, created_by_membership_id AS createdByMembershipId FROM crm_recovery_enrollments WHERE tenant_id = ? AND status = 'planned' AND COALESCE(reactivation_at, eligible_at) <= ? ORDER BY COALESCE(reactivation_at, eligible_at) LIMIT ?")
      .bind(tenantId, now.getTime(), limit).all<Omit<RecoveryEnrollment, "eligibleAt" | "reactivationAt" | "createdAt"> & { eligibleAt: number; reactivationAt: number; createdAt: number }>();
    return rows.results.map((row) => ({ ...row, eligibleAt: new Date(row.eligibleAt), reactivationAt: new Date(row.reactivationAt), createdAt: new Date(row.createdAt) }));
  }
  ...
  async addRecoveryTouch(input: { tenantId: string; enrollmentId: string; leadId: string; contactId: string; dueAt: Date; purpose: string; requestedChannel: "whatsapp" | "rich"; gateKey: string; createdAt: Date; createdByMembershipId: string }) {
    await this.db.prepare("INSERT OR IGNORE INTO crm_scheduled_touches (id, tenant_id, lead_id, contact_id, due_at, purpose, requested_channel, status, gate_key, created_at, created_by_membership_id, version) VALUES (?, ?, ?, ?, ?, ?, ?, 'planned', ?, ?, ?, 1)")
      .bind(id(), input.tenantId, input.leadId, input.contactId, input.dueAt.getTime(), input.purpose, input.requestedChannel, input.gateKey, input.createdAt.getTime(), input.createdByMembershipId).run();
  }
}

function isConstraintConflict(error: unknown): boolean {
  const message = error instanceof Error ? error.message : String(error);
  return /(?:unique|constraint)/iu.test(message);
}
