...
import { clearSessionCookie, createCsrfCookie } from "../auth/session";
...
export async function routeAuthRequest(context: RequestContext): Promise<Response | undefined> {
  ...
  if (path === "/api/v1/auth/logout") {
    ...
    headers.set("Set-Cookie", clearSessionCookie(new URL(context.request.url).protocol === "https:"));
  ...
  const csrf = createCsrfCookie(new URL(context.request.url).protocol === "https:");
